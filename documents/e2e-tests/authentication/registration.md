...
| AUTH-REG-003 | Username minimum length validation | Verify username requires minimum 12 characters | On registration form | 1. Enter username with 11 characters<br>2. Move focus to next field | Error: "Username must be at least 12 characters" | P1 | Negative | ❌ Failed | Dec 23, 2024 | BUG: Frontend accepts 11 chars, no error shown. Form submission allowed. |
| AUTH-REG-004 | Username required validation | Verify username field is required | On registration form | 1. Leave username empty<br>2. Touch field and move focus | Error: "Username is required" | P1 | Negative | ✅ Passed | Dec 23, 2024 | Error appears on blur |
| AUTH-REG-005 | Username uniqueness check | Verify duplicate username is rejected | Existing user with same username | 1. Enter username that already exists<br>2. Move focus to next field | Error: "Username is already taken" | P0 | Negative | ✅ Passed | Dec 23, 2024 | Async validation works correctly |
| AUTH-REG-006 | Email format validation | Verify email field validates format | On registration form | 1. Enter invalid email format (e.g., "test@")<br>2. Move focus to next field | Error: "Please enter a valid email" | P1 | Negative | ✅ Passed | Dec 23, 2024 | Validates format correctly |
| AUTH-REG-007 | Email required validation | Verify email field is required | On registration form | 1. Leave email empty<br>2. Touch field and move focus | Error: "Email is required" | P1 | Negative | ✅ Passed | Dec 23, 2024 | Error appears on blur |
| AUTH-REG-008 | Email uniqueness check | Verify duplicate email is rejected | Existing user with same email | 1. Enter email that already exists<br>2. Move focus to next field | Error: "Email is already registered" | P0 | Negative | ✅ Passed | Dec 23, 2024 | Async validation works correctly |
| AUTH-REG-009 | Password minimum length validation | Verify password requires minimum 12 characters | On registration form | 1. Enter password with 11 characters<br>2. Move focus to next field | Error: "Password must be at least 12 characters" | P1 | Negative | ❌ Failed | Dec 23, 2024 | BUG: Frontend accepts 11 chars, no error shown. |
| AUTH-REG-010 | Password required validation | Verify password field is required | On registration form | 1. Leave password empty<br>2. Touch field and move focus | Error: "Password is required" | P1 | Negative | ✅ Passed | Dec 23, 2024 | Error appears on blur |
| AUTH-REG-011 | Confirm password matching | Verify passwords must match | On registration form | 1. Enter password: "TestPassword123"<br>2. Enter confirm password: "DifferentPass123"<br>3. Submit form | Error: "Passwords do not match" | P0 | Negative | ✅ Passed | Dec 23, 2024 | Button disabled when passwords mismatch. No visual error message shown (UX improvement needed). |
| AUTH-REG-012 | Password visibility toggle | Verify password can be shown/hidden | On registration form with password entered | 1. Enter password<br>2. Click eye icon toggle | Password text visibility toggles between hidden (dots) and visible (plain text) | P2 | Regression | ✅ Passed | Dec 23, 2024 | Toggle icon changes correctly |
...
| AUTH-REG-015 | Loading state during submission | Verify loading indicator during form submission | Valid form data entered | 1. Fill all fields with valid data<br>2. Click "Create Account" | Button shows "Creating Account..." with loading state | P2 | Regression | ✅ Passed | Dec 23, 2024 | Button text changes and disables during submission |
