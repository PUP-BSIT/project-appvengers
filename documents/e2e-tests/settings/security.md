...
| SEC-OAUTH-001 | OAuth User Deactivate | Verify OAuth user can deactivate with email confirmation | OAuth user logged in | 1. Click Deactivate Account 2. Enter reason 3. Enter email address 4. Submit | Account deactivated, redirected to login | P0 | E2E | ⏳ Pending | - | New test for OAuth users without password |
| SEC-OAUTH-002 | OAuth User Delete | Verify OAuth user can delete with email confirmation | OAuth user logged in | 1. Click Delete Account 2. Enter reason 3. Enter email address 4. Submit | Account soft-deleted, redirected to login | P0 | E2E | ⏳ Pending | - | New test for OAuth users without password |
| SEC-OAUTH-003 | OAuth Email Mismatch | Verify OAuth user blocked with wrong email | OAuth user, modal open | 1. Enter reason 2. Enter wrong email | Delete button disabled, "Email does not match" error | P1 | Negative | ⏳ Pending | - | OAuth security validation |
| SEC-OAUTH-004 | OAuth Email Confirmation UI | Verify email input shown instead of password for OAuth | OAuth user logged in | 1. Open deactivate/delete modal | Email confirmation field shown, password field hidden | P1 | E2E | ⏳ Pending | - | UI adapts based on user type |
| SEC-OAUTH-005 | Local User Still Uses Password | Verify local user still sees password field | Local user logged in | 1. Open deactivate/delete modal | Password field shown, email confirmation hidden | P1 | Regression | ⏳ Pending | - | Ensures backward compatibility |
